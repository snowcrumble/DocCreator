...
const bool do_bleedThrough = false;
const int bleedThrough_minIter = 10;
const int bleedThrough_maxIter = 30;

const bool do_charDeg = true;
const int charDeg_minLevel = 9;
const int charDeg_maxLevel = 10;

const bool do_phantom = true;
const QString phantom_patternsPath =
  QDir(PATH_IMAGES).absoluteFilePath("phantomPatterns");

const bool do_gradientDomain = true;
const QString gradientDomain_stainImagesPath =
  QDir(PATH_IMAGES).absoluteFilePath("stainImages/images");

const bool do_blur = false;
const int blur_minIntensity = 1;
const int blur_maxIntensity = 3;

const bool do_shadow = false;

const bool do_holes = false;
...
main(int argc, char *argv[])
{
  ...
  if (do_holes) {
    hole_patterns_dirs[0] =
      QDir(PATH_IMAGES).absoluteFilePath("holePatterns/centerHoles");
    hole_patterns_dirs[1] =
      QDir(PATH_IMAGES).absoluteFilePath("holePatterns/borderHoles");
    hole_patterns_dirs[2] =
      QDir(PATH_IMAGES).absoluteFilePath("holePatterns/cornerHoles");
  ...
  for (int i = 0; i < numImages; ++i) {
    ...
    const QString imageFilename =
      QDir(inputImageDirectory).absoluteFilePath(imageList[i]);
    ...
    if (do_bleedThrough) {
      ...
      const QString versoFilename =
        QDir(inputImageDirectory).absoluteFilePath(imageList[versoIndex]);
      QImage versoImg(versoFilename);
      if (versoImg.isNull()) {
        qDebug() << "Warning: unable to load file: " << imageFilename;
      } else {
        const int nbIter =
          random_in_range(bleedThrough_minIter, bleedThrough_maxIter);
        const QImage imgBleed =
          dc::BleedThrough::bleedThrough(currImg, versoImg, nbIter);
        if (cumulate) {
          currImg = imgBleed;
          suffixe += "_bt";
        } else {
    ...
    if (do_phantom) {
      const dc::PhantomCharacter::Frequency frequency =
        (dc::PhantomCharacter::Frequency)random_in_range(0, 2);
      QImage imgPhant = dc::PhantomCharacter::phantomCharacter(
        currImg, frequency, phantom_patternsPath);
      if (cumulate) {
        currImg = imgPhant;
        suffixe += "_ph";
      } else {
    ...
    if (do_gradientDomain) {
      ...
      QImage imgGDD = dc::GradientDomainDegradation::degradation(
        currImg,
        gradientDomain_stainImagesPath,
        numStains,
        insertType,
        doRotations);
      if (cumulate) {
    ...
    if (do_blur) {
      const dc::BlurFilter::Method m =
        (dc::BlurFilter::Method)random_in_range(0, 2);
      const int intensity =
        1 + 2 * random_in_range(blur_minIntensity, blur_maxIntensity);
    ...
    if (do_shadow) {
      const dc::ShadowBinding::Border border =
        (dc::ShadowBinding::Border)random_in_range(0, 3);
      const int distance = random_in_range(
        0, int(0.25 * std::min<int>(currImg.width(), currImg.height())));
      const float intensity = random_in_range(0, 255) / 255.f;
      const float angle = random_in_range(0, 90);
      QImage imgShad = dc::ShadowBinding::shadowBinding(
        currImg, border, distance, intensity, angle);
      if (cumulate) {
        currImg = imgShad;
        suffixe += "_sh";
      } else {
    ...
    if (do_holes) {
      ...
      for (int j = 0; j < numHoles; ++j) {
        const dc::HoleDegradation::HoleType holeType =
          (dc::HoleDegradation::HoleType)random_in_range(0, 2);
        const QStringList &patterns = hole_patterns[(int)holeType];
        const int holeIndex = random_in_range(0, patterns.size() - 1);
        const QString holePatternFilename =
          QDir(hole_patterns_dirs[(int)holeType])
            .absoluteFilePath(patterns[holeIndex]);
        ...
        imgHole = dc::HoleDegradation::holeDegradation(
          imgHole, holePattern, ratioOutside, size, holeType, side, color);
      }
